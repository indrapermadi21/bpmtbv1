...
class Iuitpp extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('ppu/m_iuitpp');
    }
    ...
    function index() {
        if ($this->session->userdata('is_log') != "") {
            ...
            $d['listIuitpp'] = $this->m_iuitpp->listIuitpp($d['tgl_awal'], $d['tgl_akhir']);

            $d['listKecamatan'] = $this->m_global->getKecamatan();
            //$d['listKelurahan'] = $this->m_global->getKelurahan();
            $d['content'] = 'ppu/iuitpp/lf_iuitpp';
    ...
    function getIuitpp() {
        $result = $this->m_iuitpp->getIuitpp();
        echo json_encode(array('data' => $result, 'success' => true));
    }

    function save() {
        if ($this->m_iuitpp->saved()) {
